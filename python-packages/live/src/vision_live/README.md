...
```bash
uv run python -m vision_live.agent --cdp-url http://localhost:9222 --audio-device "CABLE Input"
```
...
curl -X POST http://localhost:8000/live/start \
  -H "Content-Type: application/json" \
  -d '{"script": "python-packages/live/src/vision_live/example_script.yaml"}'
```
...
```bash
uv run python -m vision_live.try_voices
uv run python -m vision_live.try_voices --text "感谢大家的支持！" --voices Kore Sulafat Aoede
```

## 运行测试

```bash
uv run pytest python-packages/live/ -v
```

## 文件索引

```
python-packages/live/src/vision_live/
...
# 查询测试
uv run python -m vision_live.rag_cli query <plan_id> "产品介绍要点"

...
from vision_live.rag_cli import (
    _save_meta,
    compute_file_hash,
    diff_sources,
    load_meta,
...
def test_load_save_meta_roundtrip(tmp_path: Path):
    ...
    assert load_meta(path) == data


def test_load_meta_missing_returns_empty(tmp_path: Path):
    assert load_meta(tmp_path / "none.json") == {}
...
def test_diff_sources_detects_added_and_changed():
    ...
    added_or_changed, removed = diff_sources(prev, current)

...
from vision_live.proto_douyin import (
    ChatMessage,
    GiftMessage,
    MemberMessage,
    Message,
...
def _rich_user(
    ...
    from vision_live.proto_douyin import FansClub, FansClubData, FollowInfo, PayGrade, User
...
def _chat_payload(user: str, content: str) -> bytes:
    from vision_live.proto_douyin import User
...
def _gift_payload(
    ...
    from vision_live.proto_douyin import GiftStruct, User
...
def _like_payload(user: str, count: int) -> bytes:
    from vision_live.proto_douyin import LikeMessage, User

    return bytes(LikeMessage(user=User(nick_name=user), count=count))


def _social_payload(user: str, action: int = 1) -> bytes:
    from vision_live.proto_douyin import SocialMessage, User
...
def _fansclub_payload(user: str, content: str, type_: int) -> bytes:
    from vision_live.proto_douyin import FansclubMessage, User
...
class TestUserInfo:
    ...
    def test_chat_user_info_basic_fields(self):
        from vision_live.proto_douyin import ChatMessage
    ...
    def test_gift_has_user_info_and_combo(self):
        from vision_live.proto_douyin import GiftStruct
    ...
    def test_follow_vs_share_via_action(self):
        from vision_live.proto_douyin import SocialMessage
    ...
    def test_like_has_user_info(self):
        from vision_live.proto_douyin import LikeMessage
    ...
    def test_fansclub_has_user_info(self):
        from vision_live.proto_douyin import FansclubMessage

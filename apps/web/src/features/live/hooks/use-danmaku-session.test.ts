...
import { useDanmakuSession } from './use-danmaku-session'
...
describe('useDanmakuSession', () => {
  ...
  it('sets error state when start fails', async () => {
    ...
    const { result } = renderHook(() => useDanmakuSession())
    await waitFor(() => expect(mockApiFetch).toHaveBeenCalled())

    await act(async () => { await result.current.start() })

    expect(result.current.error).toBe('Failed to start danmaku')
  })
  ...
  it('sets error state when stop fails', async () => {
    ...
    expect(result.current.error).toBe('Failed to stop danmaku')
  })
})
